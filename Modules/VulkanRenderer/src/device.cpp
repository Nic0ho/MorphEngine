...
void VulkanPhysicalDevices::Init(const VkInstance& Instance, const VkSurfaceKHR& Surface)
{
    ...
    for(u32 i = 0; i < NumDevices; i++)
    {
        ...
        CHECK_VK_RESULT(res, "vkGetPhysicalDeviceSurfaceFormatKHR (2)\n");
        ...
        CHECK_VK_RESULT(res, "vkGetPhysicalDeviceSurfacePreseentModesKHR (1) error\n");
...
u32 VulkanPhysicalDevices::SelectDevice(VkQueueFlags RequiredQueueType, bool SupportsPresent)
{
    ...
    MORPH_ERROR("Required queue type %x and supports present %d not found\n", RequiredQueueType, SupportsPresent);
...
{
    if(m_devIndex < 0)
    { MORPH_ERROR("A physical device has not been selected\n"); }
