...
static bool CompileShader(VkDevice Device, glslang_stage_t Stage, const char* pShderCode, Shader& ShaderModule)
{
    ...
    if(spirv_messages) fprintf(stderr, "SPIR-V message: '%s'", spirv_messages);
...
static glslang_stage_t ShaderStageFromFilename(const char* pFilename)
{
    ...
    printf("Unkrown shader stage in '%s'\n", pFilename);
    exit(1);
...
VkShaderModule CreateShaderModuleFromText(VkDevice Device, const char* pFilename)
{
    ...
    if(Success)
    {
        ...
        WriteBinaryFile(BinaryFilename.c_str(), ShaderModule.SPIRV.data(), (int)ShaderModule.SPIRV.size() * sizeof(u32));
    }
